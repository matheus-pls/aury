...
export default function CoupleMode() {
  ...
  // Usuário não-Premium e não membro de nenhum grupo: mostrar tela de promoção exclusiva
  if (!activeGroup && !isPremium) {
    return (
      <div className="min-h-[80vh] relative overflow-hidden flex flex-col items-center justify-center px-4 py-10">
        {/* Background gradient */}
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(225,29,72,0.18) 0%, transparent 70%)" }} />
        ...
        {/* Animated heart */}
        <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200, damping: 14 }} className="relative mb-6">
          <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ repeat: Infinity, duration: 2.2, ease: "easeInOut" }} className="w-28 h-28 bg-gradient-to-br from-rose-400 to-pink-600 rounded-3xl flex items-center justify-center shadow-2xl shadow-rose-500/30">
            <Heart className="w-14 h-14 text-white fill-white" />
          </motion.div>
          <motion.div animate={{ opacity: [0.4, 1, 0.4], scale: [0.95, 1.05, 0.95] }} transition={{ repeat: Infinity, duration: 2.2, ease: "easeInOut" }} className="absolute inset-0 rounded-3xl border-2 border-rose-400/40" />
        </motion.div>

        {/* Title */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="text-center mb-8 max-w-sm">
          <h1 className="text-4xl font-bold text-foreground mb-3">Modo Casal</h1>
          <p className="text-muted-foreground text-base leading-relaxed">Um espaço só de vocês dois, sem julgamento e com muito mais harmonia 💕</p>
        </motion.div>

        {/* Two cards */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg mb-8">
          {/* Card 1 — Premium */}
          <div className="rounded-2xl border border-rose-500/30 bg-rose-500/5 p-5 flex flex-col gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 leading-relaxed">Assine o Premium e crie o espaço do casal</p>
            </div>
            <p className="text-rose-500 font-bold text-base">R$ 9,90<span className="text-xs font-normal text-muted-foreground">/mês</span></p>
            <Button onClick={() => navigate(createPageUrl("Upgrade"))} className="w-full h-10 bg-gradient-to-r from-rose-400 to-pink-500 text-white text-sm font-semibold mt-auto">
              Assinar Premium
            </Button>
          </div>
          ...
          <div className="rounded-2xl border border-rose-500/20 bg-card p-5 flex flex-col gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-400 to-pink-500 flex items-center justify-center shadow-md text-lg">
              💌
            </div>
            <div>
              <p className="font-bold text-foreground text-sm">Tenho um Código</p>
              <p className="text-xs text-muted-foreground mt-1 leading-relaxed">Meu parceiro(a) me convidou para o espaço dele(a)</p>
            </div>
            <div className="flex-1" />
            <Button variant="outline" onClick={() => setIsJoinGroupOpen(true)} className="w-full h-10 border-rose-300 text-rose-500 hover:bg-rose-50 text-sm font-semibold">
              💌 Digitar código
            </Button>
          </div>
        </motion.div>
        ...
        {/* Bullets */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }} className="flex flex-col gap-2 w-full max-w-sm">
          {["💳 Gastos compartilhados sem surpresas", "🎯 Sonhos que vocês constroem juntos", "📊 Transparência total entre vocês dois"].map((item, i) => (
            <div key={i} className="p-3 bg-rose-500/10 rounded-xl border border-rose-500/15 text-sm text-rose-400 font-medium">{item}</div>
          ))}
        </motion.div>
              ...
              {joinSuccess ? (
                ...
                <>
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button className="flex-1 h-11 bg-gradient-to-r from-rose-400 to-pink-500 text-white font-semibold" onClick={handleJoinGroup} disabled={!joinCode.trim()}>
                      Entrar com amor 💕
                    </Button>
                  </div>
                </>
