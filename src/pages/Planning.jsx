  ...
  return (
    <div className="space-y-6 pb-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-2xl lg:text-3xl font-bold text-foreground">Como você quer se organizar?</h1>
        <p className="text-muted-foreground mt-1">Escolha o que faz sentido pro seu momento</p>
      </motion.div>
      ...
      {isPremium === false && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 flex items-center gap-3">
            ...
            <p className="text-sm text-amber-500/90 flex-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {PLANNING_MODULES.map((module, index) => {
          ...
          return (
            ...
            >
              <Card className={`cursor-pointer transition-all border group bg-card ${isLocked ? 'border-dashed border-border opacity-80' : 'border-border hover:shadow-aury hover:border-[#5FBDBD]'}`}>
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${isLocked ? 'from-muted to-muted' : module.color} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform shadow-md`}>
                      {isLocked ? <Lock className="w-8 h-8 text-muted-foreground" /> : <Icon className="w-8 h-8 text-white" />}
                    </div>
                    ...
                    {isLocked
                      ...
                      : <ChevronRight className="w-5 h-5 text-muted-foreground group-hover:text-[#5FBDBD] group-hover:translate-x-1 transition-all" />
                    }
                  </div>
                </CardContent>
              </Card>
